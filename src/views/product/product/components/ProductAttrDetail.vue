      ...
      <el-form-item label="商品规格：">
        <el-card shadow="never" class="cardBg">
          <div v-for="(productAttr,idx) in selectProductAttr" :key="productAttr.id">
            {{productAttr.name}}：
            <el-checkbox-group v-if="productAttr.hand_add_status === 0" v-model="selectProductAttr[idx].values">
      ...
      <el-form-item label="商品参数：">
        <el-card shadow="never" class="cardBg">
          <div v-for="(item,index) in selectProductParam" :class="{littleMarginTop:index!==0}" :key="index">
            ...
            <el-select v-if="item.input_type===1" class="paramInput" v-model="selectProductParam[index].value">
              <el-option
                v-for="item in getParamInputList(item.input_list)"
                :key="item"
                :label="item"
                :value="item">
              </el-option>
...
<script>
  ...
  export default {
    ...
    components: {SingleImage, Tinymce, MultiUpload},
    props: {
      value: Object,
      isEdit: {
        type: Boolean,
    ...
    computed: {
      ...
      selectProductPics:{
        get:function () {
          ...
          if(this.value.album_pics === undefined || this.value.album_pics == null || this.value.album_pics === ''){
            return pics;
          }
          let albumPics = this.value.album_pics.split(',');
        ...
        set:function (newValue) {
          if (newValue == null || newValue.length === 0) {
            ...
            this.value.album_pics = '';
            if (newValue.length > 1) {
              for (let i = 1; i < newValue.length; i++) {
                this.value.album_pics += newValue[i];
    ...
    created() {
      this.getProductCategoryAttrList();
    },
    watch: {
    ...
    methods: {
      ...
      getProductCategoryAttrList() {
        let param = {page_num: 1, page_size: 100};
          ...
          if (type === 0) {
            ...
            for (let i = 0; i < list.length; i++) {
              ...
              this.selectProductAttr.push({
                id: list[i].id,
                name: list[i].name,
                hand_add_status: list[i].hand_add_status,
                input_list: list[i].input_list,
                values: values,
                options: options
              });
      ...
      getEditAttrOptions(id) {
        let options = [];
        if (this.value.product_attr_value == null) {
          return options;
        }

        for (let i = 0; i < this.value.product_attr_value.length; i++) {
          let attrValue = this.value.product_attr_value[i];
          // console.log(id, attrValue.product_attribute_id, "getEditAttrOptions")
      ...
      getEditAttrValues(index) {
        let values = new Set();
        if (index === 0) {
          for (let i = 0; i < this.value.sku_stock.length; i++) {
            let sku = this.value.sku_stock[i];
            
            let spData = JSON.parse(sku.sp_data);
      ...
      getEditParamValue(id){
        ...
        for(let i = 0;i < this.value.product_attr_value.length; i++){
          if(id === this.value.product_attr_value[i].product_attribute_id){
            return this.value.product_attr_value[i].value;
          }
        }
      },
      ...
      handleSyncProductSkuPrice(){
        this.$confirm('将同步第一个sku的价格到所有sku,是否继续', '提示', {
          ...
          if(this.value.sku_stock !== null && this.value.sku_stock.length > 0){
            let tempSkuList = [];
            tempSkuList = tempSkuList.concat(tempSkuList,this.value.sku_stock);
            let price = this.value.sku_stock[0].price;
            for(let i = 0;i < tempSkuList.length; i++){
              tempSkuList[i].price = price;
            }
            this.value.sku_stock = [];
            this.value.sku_stock = this.value.sku_stock.concat(this.value.sku_stock,tempSkuList);
          }
        });
      },
      handleSyncProductSkuStock(){
        this.$confirm('将同步第一个sku的库存到所有sku,是否继续', '提示', {
          ...
          if(this.value.sku_stock !== null && this.value.sku_stock.length > 0){
            let tempSkuList = [];
            tempSkuList = tempSkuList.concat(tempSkuList,this.value.sku_stock);
            let stock = this.value.sku_stock[0].stock;
            let lowStock = this.value.sku_stock[0].low_stock;
            for(let i = 0;i < tempSkuList.length; i++){
              tempSkuList[i].stock = stock;
              tempSkuList[i].low_stock = lowStock;
            }
            this.value.sku_stock = [];
            this.value.sku_stock = this.value.sku_stock.concat(this.value.sku_stock,tempSkuList);
          }
      ...
      refreshProductSkuList() {
        this.value.sku_stock = [];
        let skuList = this.value.sku_stock;
        ...
        if (this.selectProductAttr.length === 1) {
          ...
          for (let i = 0; i < attr0.values.length; i++) {
            if (attr1.values.length === 0) {
              skuList.push({
                sp_data: JSON.stringify([{key:attr0.name,value:attr0.values[i]}])
              });
              continue;
            }
      ...
      getProductSkuPic(name){
        for(let i = 0;i < this.value.sku_stock.length; i++){
          let spData = JSON.parse(this.value.sku_stock[i].sp_data);
          if(name === spData[0].value){
            return this.value.sku_stock[i].pic;
          }
        }
        return null;
      },
      ...
      mergeProductAttrValue() {
        this.value.product_attr_value = [];
        for (let i = 0; i < this.selectProductAttr.length; i++) {
          let attr = this.selectProductAttr[i];
          if (attr.hand_add_status === 1 && attr.options != null && attr.options.length > 0) {
            this.value.product_attr_value.push({
      ...
      mergeProductAttrPics() {
        for (let i = 0; i < this.selectProductAttrPics.length; i++) {
          for (let j = 0; j < this.value.sku_stock.length; j++) {
            let spData = JSON.parse(this.value.sku_stock[j].sp_data);
            if (spData[0].value === this.selectProductAttrPics[i].name) {
              this.value.sku_stock[j].pic = this.selectProductAttrPics[i].pic;
            }
          }
        }
      },
